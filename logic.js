/** LOGIC: 4-Stack Limit, Action Menu & New Commands (TRULY FULL VERSION) */
class Game {
    constructor() {
        this.units = [];
        this.map = [];
    ...
    startCampaign() {
        ...
        setTimeout(() => {
            if (Renderer.dealCards) Renderer.dealCards(['rifleman', 'tank_pz4', 'gunner', 'scout', 'tank_tiger']);
        }, 500);
    }
    ...
    async actionAttack(a, d) {
        ...
        for (let i = 0; i < shots; i++) {
            ...
            if (window.VFX) VFX.addProj({
                ...
                speed: 0.1,
                progress: 0,
                arcHeight: (w.type.includes('shell') ? 100 : 0),
                onHit: null
            });
    ...
    generateMap() {
        ...
        for (let i = 0; i < 140; i++) {
            ...
            if (Math.random() < 0.05 && walkers.length < 5) walkers.push(next);
            else walkers[wIdx] = next;
        }
        for (let i = 0; i < 3; i++) {
    ...
    swapWeapon() {}
    checkPhaseEnd() {
        if (this.units.filter(u => u.team === 'player' && u.hp > 0 && u.ap > 0).length === 0 && this.state === 'PLAY') this.endTurn();
    }
    setStance(s) {
        if (this.selectedUnit && this.selectedUnit.ap >= 1 && !this.selectedUnit.def.isTank) {
            this.selectedUnit.ap--;
            this.selectedUnit.stance = s;
            this.refreshUnitState(this.selectedUnit);
            this.checkPhaseEnd();
        }
    }
    endTurn() {
        ...
        this.units.filter(u => u.team === 'player' && u.hp > 0 && u.skills.includes("Mechanic")).forEach(u => {
            ...
            if (u.hp < u.maxHp) {
                u.hp = Math.min(u.maxHp, u.hp + c * 20);
                this.log(`${u.name} 修理`);
            }
        });
        setTimeout(async () => {
                ...
                if (distToTarget <= w.rng && e.ap >= w.ap) {
                    ...
                    if (p.length > 0) {
                        ...
                        if (this.map[next.q][next.r].cost <= e.ap) {
                            e.q = next.q;
                            e.r = next.r;
                            e.ap -= this.map[next.q][next.r].cost;
    ...
    healSurvivors() {
        this.units.filter(u => u.team === 'player' && u.hp > 0).forEach(u => {
            const t = Math.floor(u.maxHp * 0.8);
            if (u.hp < t) u.hp = t;
        });
        this.log("治療完了");
    }
    ...
    checkLose() {
        if (this.units.filter(u => u.team === 'player' && u.hp > 0).length === 0) document.getElementById('gameover-screen').style.display = 'flex';
    }
    isValidHex(q, r) {
    ...
    cubeRound(c) {
        ...
        const x_diff = Math.abs(rx - c.x),
            y_diff = Math.abs(ry - c.y),
            z_diff = Math.abs(rz - c.z);
