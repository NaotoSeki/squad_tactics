/** * PHASER BRIDGE (Aerial Logic Connected) */
let phaserGame = null;
...
function getCardTextureKey(scene, type) {
    ...
    if (!scene.textures.exists(key)) scene.textures.addCanvas(key, window.createCardIcon(type));
    return key;
}
...
class Card extends Phaser.GameObjects.Container {
    constructor(scene, x, y, type) {
        ...
        if(type === 'aerial' && scene.textures.exists('card_img_bomb')) icon.setDisplaySize(120, 80); else icon.setScale(1/HIGH_RES_SCALE);
    ...
    burnAndConsume(hex) {
        ...
        this.scene.tweens.add({
            ...
            onUpdate: () => {
                ...
                for(let i=0; i<8; i++) { 
                    const randX = (Math.random() - 0.5) * 140; const wx = this.x + (randX * cos - burnLineY * sin); const wy = this.y + (randX * sin + burnLineY * cos);
                    UIVFX.addFire(wx, wy); if(Math.random()<0.3) UIVFX.addSmoke(wx, wy);
                }
            ...
            onComplete: () => {
                ...
                try {
                    if (type === 'aerial') {
                        ...
                        if (main) main.triggerBombardment(hex); // ★爆撃開始
                    } 
...
class MainScene extends Phaser.Scene {
    ...
    triggerBombardment(hex) { 
        this.time.delayedCall(500, () => { // 一呼吸
            const targetPos = Renderer.hexToPx(hex.q, hex.r);
            // ★爆撃エフェクト開始時にターゲットHexを渡す
...
window.VFX = { 
    ...
    addBombardment(scene, tx, ty, hex) { // ★hex引数追加
