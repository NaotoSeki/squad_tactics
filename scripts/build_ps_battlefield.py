#!/usr/bin/env python3
"""PS正本クロップ -> 本編用の戦場キャンバス(背景PNG + 広域地形テーブル)。
...
# 広域盤の行構成。10行×12列。一般的な本番戦場ペインより横長にして、
# cover表示のままでも俯瞰縮小と左右パンの余地を残す。
BOARD_ROWS: list[tuple[int, int]] = [
    (5, 6), (6, 5), (7, 5), (8, 4), (9, 4),
    (10, 3), (11, 3), (12, 2), (13, 2), (14, 1),
]
ROW_LEN = 12
...
def write_registry(out_dir: Path) -> Path:
    ...
    for path in sorted(out_dir.glob("*.json")):
        ...
        if pixel_ratio is not None:
            ...
            # and object ledgers therefore remain in the logical canvas space.
