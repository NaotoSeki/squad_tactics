/** LOGIC GAME: Core Logic (Interaction delegated to logic_interaction.js) */
...
class Game {
    constructor() {
        ...
        // Interaction states (Synced with InteractionManager)
        this.path = [];
        this.reachableHexes = [];
        this.attackLine = [];
    ...
    // --- Delegation to InteractionManager ---
    handleClick(p) { this.interaction.handleClick(p); }
    handleRightClick(mx, my, hex) { this.interaction.handleRightClick(mx, my, hex); }
    handleHover(p) { this.interaction.handleHover(p); }
    setMode(mode) { this.interaction.setMode(mode); }
    clearSelection() { this.interaction.clearSelection(); }
    onUnitClick(u) { this.interaction.onUnitClick(u); }
    // ----------------------------------------
    ...
    startCampaign() {
        ...
        this.selectedUnit = null; this.interaction.clearSelection();
    ...
    setStance(s) {
        ...
        u.ap -= cost; u.stance = s; this.interaction.refreshUnitState(u); this.ui.hideActionMenu(); if (window.Sfx) Sfx.play('click');
    }
    ...
    reloadWeapon(isManual = false) {
        ...
        if (u.def.isTank) {
            ...
            if (window.Sfx) Sfx.play('reload'); this.interaction.refreshUnitState(u); if (isManual) this.ui.hideActionMenu();
        } else {
            const cost = w.rld || 1; const magIdx = u.bag.findIndex(i => i && i.type === 'ammo' && i.ammoFor === w.code);
            if (u.ap < cost || magIdx === -1) return;
            u.bag[magIdx] = null; u.ap -= cost; w.current = w.cap; if (window.Sfx) Sfx.play('reload'); this.interaction.refreshUnitState(u); this.ui.hideActionMenu();
        }
    }

    actionRepair() { const u = this.selectedUnit; if (!u || u.team !== 'player' || u.ap < 2 || !u.hands || !u.hands.isBroken) return; u.ap -= 2; u.hands.isBroken = false; if (window.Sfx) Sfx.play('reload'); this.interaction.refreshUnitState(u); this.ui.hideActionMenu(); }
    actionHeal() { const u = this.selectedUnit; if (!u || u.team !== 'player' || u.ap < 2) return; const target = this.getUnitsInHex(u.q, u.r).filter(t => t.team === u.team && t.hp < t.maxHp).sort((a,b) => (a.hp/a.maxHp)-(b.hp/b.maxHp))[0]; if (!target) return; u.ap -= 2; target.hp = Math.min(target.maxHp, target.hp + 30); if (window.VFX) { const p = Renderer.hexToPx(u.q, u.r); window.VFX.add({ x: p.x, y: p.y - 20, vx: 0, vy: -1, life: 30, color: "#0f0", size: 4, type: 'spark' }); } this.interaction.refreshUnitState(u); this.ui.hideActionMenu(); }
    ...
    async actionMelee(a, d) {
        ...
        this.interaction.refreshUnitState(a); this.checkPhaseEnd();
    }
    ...
    async actionAttack(a, d) {
        ...
        if (!a.def.isTank && w.current <= 0) { const magIdx = a.bag.findIndex(i => i && i.type === 'ammo' && i.ammoFor === w.code); if (magIdx !== -1 && a.ap >= (w.rld || 1)) { a.ap -= (w.rld || 1); a.bag[magIdx] = null; w.current = w.cap; if (window.Sfx) Sfx.play('reload'); this.interaction.refreshUnitState(a); await new Promise(r => setTimeout(r, 600)); } else return; }
        if (a.def.isTank && w.current <= 0 && this.tankAutoReload) { if (w.reserve > 0 && a.ap >= 1) { a.ap -= 1; w.reserve--; w.current = 1; if (window.Sfx) Sfx.play('reload'); this.interaction.refreshUnitState(a); } else return; }
        ...
        setTimeout(() => { 
            ...
            this.interaction.refreshUnitState(a); if (!(a.ap >= (w?w.ap:0))) { this.interaction.setMode('SELECT'); this.checkPhaseEnd(); }
        }, 800);
    }
    ...
    async actionMove(u, p) { this.state = 'ANIM'; for (let s of p) { u.ap -= this.map[s.q][s.r].cost; u.q = s.q; u.r = s.r; if (window.Sfx) Sfx.play('move'); await new Promise(r => setTimeout(r, 180)); } this.checkReactionFire(u); this.state = 'PLAY'; this.interaction.refreshUnitState(u); this.checkPhaseEnd(); }
